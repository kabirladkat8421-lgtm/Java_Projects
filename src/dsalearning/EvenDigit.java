...
public class EvenDigit {
	...
	public static int numFinder(int[] nums) {
		...
		return count;//it will return the total number count or we can also say numbers of even come in array total count that will return;
	}

	static boolean even(int num) {
		int NumberOfDigitinNum = digit(num);//total number of count will be passed digit because it's returning count;
		if (NumberOfDigitinNum % 2 == 0) {//NumberOfdigitInNum(count)%2 will return it's remainder and it will check if it's 0 then it even but remainder is 1 it will return false that's means it's odd 
			return true; //if it's true execution contineu and it will go to numFinder, but it's false execution stop.
		}
		return false;
	}
